...
from src.utils import per_date_sign_agreement, sign_label
...
class ModelEvaluator:
    ...
    def save_results(
        ...
        try:
            ...
            if os.path.exists(config_path):
                ...
                dest_config_path = os.path.join(self.output_dir, config_filename)
                shutil.copy2(config_path, dest_config_path)
                print(f"配置文件: {dest_config_path}")
    ...
    def _save_feature_importance(self, model):
        ...
        try:
            ...
            # 转换为列表，包含所有特征（即使importance为0）
            importance_items = []
            for fname in feature_names:
                importance_value = imp_dict.get(fname, 0.0)
                importance_items.append((fname, importance_value))
            ...
            print(f"特征重要性CSV: {fi_csv}")
            
            # 2. 生成可视化图表（Top N特征）
            self._plot_feature_importance(importance_items)
    ...
    def _plot_feature_importance(self, importance_items: list, top_n: int = 30):
        ...
        Args:
            importance_items: [(feature_name, importance), ...] 列表
            top_n: 显示前N个重要特征
        """
        try:
            # 只取Top N个特征
            top_items = importance_items[:min(top_n, len(importance_items))]
            ...
            # 转换为DataFrame方便绘图
            fi_df = pd.DataFrame(top_items, columns=['feature', 'importance'])
            
            # 创建图表
            fig_size = self.viz_config.get('figure_size', [10, 6])
            fig_height = max(8, len(top_items) * 0.3)  # 根据特征数量调整高度
            plt.figure(figsize=(fig_size[0], fig_height))
            
            # 绘制水平柱状图（从上到下降序）
            plt.barh(range(len(fi_df)), fi_df['importance'], align='center')
            plt.yticks(range(len(fi_df)), fi_df['feature'])
            plt.xlabel('Importance (Gain)')
            plt.ylabel('Feature')
            plt.title(f'Top {len(top_items)} Feature Importance')
            plt.gca().invert_yaxis()  # 最重要的在顶部
            plt.tight_layout()
            
            # 保存图表
            fig_file = os.path.join(self.output_dir, "feature_importance.png")
            plt.savefig(fig_file, dpi=self.viz_config.get('dpi', 100), bbox_inches='tight')
            print(f"特征重要性图表: {fig_file}")
            plt.close()
            
        except Exception as e:
            import traceback
            print(f"绘制特征重要性图表失败: {e}")
            traceback.print_exc()
