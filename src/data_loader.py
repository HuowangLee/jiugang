...
from src.utils import safe_read_csv, quick_check_columns
...
class DataLoader:
    ...
    def _read_and_merge(self, feat_path: str, lab_path: str) -> pd.DataFrame:
        ...
        # 如果启用样本权重且price_diff列存在，也保留它
        sample_weight_config = self.config.config.get('sample_weight', {})
        if sample_weight_config.get('enabled', False):
            if 'price_diff' in lab.columns and self.label_column != 'price_diff':
                label_columns_to_keep.append('price_diff')
                print('样本权重已启用，保留price_diff列用于权重计算')
        ...
        print(f"合并示例:")
        display_cols = ['info_date', 'info_hour', self.label_column]
        if 'price_diff' in df.columns and self.label_column != 'price_diff':
